...
public class depth_first_search_recursive {
    ...
    private static ArrayList<Integer> dfsTravesal(ArrayList<ArrayList<Integer>> adj, int n) {
        ...
        for (int i = 0; i < n; i++) {
            if (vis[i] == false) {
                dfs(adj, storeDfs, i, vis);
            }
        }
        return storeDfs;
    }

    private static void dfs(ArrayList<ArrayList<Integer>> adj, ArrayList<Integer> storeDfs, int node, boolean[] vis) {
        storeDfs.add(node);
        vis[node] = true;
        for (Integer it : adj.get(node)) {
            if (vis[it] == false) {
                dfs(adj, storeDfs, it, vis);
            }
        }
    }
